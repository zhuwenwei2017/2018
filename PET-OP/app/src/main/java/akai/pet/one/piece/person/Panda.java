...
public class Panda extends Person{
...
//一般数据
        ...
        bmpW = decodeResource(res, R.drawable.panda_sit).getWidth();
        bmpH = decodeResource(res, R.drawable.panda_sit).getHeight();
...
//初始化的xy
    ...
    public void onSharedPreferenceChanged(SharedPreferences sp,
                                          String key) {
        if(key.indexOf("panda_action_") != -1){
            setActionGroup(sp);
        }
    ...
    private void setActionGroup(SharedPreferences sp){
        ...
        if(sp.getBoolean("panda_action_stand", false))
            actionGroup.add(FLAG_STAND);
        if(sp.getBoolean("panda_action_walk", false)){
    ...
    private void onActionChange(int actionFlag){
        ...
        switch (actionFlag){
            case FLAG_DOWN:
                ...
                bmpPanda[0] = decodeResource(res, R.drawable.panda_down_1);
                bmpPanda[1] = decodeResource(res, R.drawable.panda_down_2);
                break;
            case FLAG_SIT:
                upOrDown = -1;//默认向下看
