...
public class Nanjing extends Person{
    ...
    private Bitmap[] bmpPanda = new Bitmap[4];
    /**标识常量*/
    ...
    public Nanjing(Context context) {
        super(context);
        paint = new Paint();
...
//一般数据
        ...
        bmpW = decodeResource(res, R.drawable.nanjing_stand).getWidth();
        bmpH = decodeResource(res, R.drawable.nanjing_stand).getHeight();
...
//初始化的xy
    ...
    public void onSharedPreferenceChanged(SharedPreferences sp,
                                          String key) {
        if(key.indexOf("nanjing_action_") != -1){
            setActionGroup(sp);
        }
    ...
    private void actionCrawl(Canvas canvas, Paint paint){
        if(x <= 0 && leftOrRight == 1
            ...
            leftOrRight *= -1;
        canvasDraw(canvas, bmpPanda[frameFlag/6%2], matrix, paint);
        frameFlag ++;
    ...
    private void actionDown(Canvas canvas, Paint paint){
        if(frameFlag <= 1){
            canvasDraw(canvas, bmpPanda[0], matrix, paint);
            int disY = (int) (screenH - bmpH/2 - bmpH*personSize/2);
            if(y < disY)
                y += speed * 3;
        }else if(frameFlag <= 3){
            canvasDraw(canvas, bmpPanda[1], matrix, paint);
        }else{
            int disY = (int) (screenH - bmpH/2 - bmpH*personSize/2);
            if(y > disY)
                y = disY;
    ...
    private void actionEat(Canvas canvas, Paint paint){
        canvasDraw(canvas, bmpPanda[frameFlag%3], matrix, paint);
        frameFlag ++;
    }
    /**
     * 动作：坐着,sit
    ...
    private void actionUp(Canvas canvas, Paint paint){
        ...
        //动作绘制
        canvasDraw(canvas, bmpPanda[frameFlag*leftOrRight], matrix, paint);
        //记录前一个点
        touchPreX = touchX;
    }
    /**
    ...
    private void actionWalk(Canvas canvas, Paint paint){
        ...
        if(frameFlag%4 == 0)
            canvasDraw(canvas, bmpPanda[0], matrix, paint);
        else if(frameFlag%4 == 1)
            canvasDraw(canvas, bmpPanda[1], matrix, paint);
        else if(frameFlag%4 == 2)
            canvasDraw(canvas, bmpPanda[2], matrix, paint);
        else
            canvasDraw(canvas, bmpPanda[3], matrix, paint);
    ...
    private void onActionChange(int actionFlag){
        ...
        switch (actionFlag){
            case FLAG_DOWN:
                ...
                bmpPanda[0] = decodeResource(res, R.drawable.nanjing_down_1);
                bmpPanda[1] = decodeResource(res, R.drawable.nanjing_down_2);
                break;
            //case FLAG_SIT:
            //    upOrDown = -1;//默认向下看
            ...
            //    break;
            case FLAG_STAND:
                bmpPanda[0] = decodeResource(res, R.drawable.nanjing_stand);
                break;
            case FLAG_UP:
                bmpPanda[0] = decodeResource(res, R.drawable.nanjing_stand);
                bmpPanda[1] = decodeResource(res, R.drawable.nanjing_up_1);
                bmpPanda[2] = decodeResource(res, R.drawable.nanjing_up_1);
                break;
            case FLAG_WALK2:
                upOrDown = Math.random()< 0.5 ? 1:-1;
            case FLAG_WALK:
                bmpPanda[0] = decodeResource(res, R.drawable.nanjing_walk_1);
                bmpPanda[1] = decodeResource(res, R.drawable.nanjing_stand);
                bmpPanda[2] = decodeResource(res, R.drawable.nanjing_walk_2);
                bmpPanda[3] = decodeResource(res, R.drawable.nanjing_stand);
                break;
        }
    }
}
